...
# v19 : FVG 15M + OB 15M + LIMIT AUTO + FUTURES OFFSET (compatible Lucid Trading MGC/GC)
    ...
    if not candidates:
        if direction == "BUY":
            if ob and ob[0] == "BULL":
                ...
                candidates.append({"fib": "OB", "limit": round(ob[2], 2), "sl": round(ob[1]-2.0, 2),
                                    "score": s, "has_fvg": fvg is not None and fvg[0]=="BULL", "has_ob": True})
            elif fvg and fvg[0] == "BULL":
                candidates.append({"fib": "FVG", "limit": round(fvg[2], 2), "sl": round(fvg[1]-2.0, 2),
                                    "score": 35, "has_fvg": True, "has_ob": False})
        else:
            if ob and ob[0] == "BEAR":
                s = 40 + (35 if fvg and fvg[0] == "BEAR" else 0)
                candidates.append({"fib": "OB", "limit": round(ob[1], 2), "sl": round(ob[2]+2.0, 2),
                                    "score": s, "has_fvg": fvg is not None and fvg[0]=="BEAR", "has_ob": True})
            elif fvg and fvg[0] == "BEAR":
                candidates.append({"fib": "FVG", "limit": round(fvg[1], 2), "sl": round(fvg[2]+2.0, 2),
                                    "score": 35, "has_fvg": True, "has_ob": False})
...
def format_limit_message(label, direction, price, limit_entry, atr_v):
    ...
    msg += "━━━━━━━━━━━━━━━━━━\n"
    msg += "📐 Zone     : " + fib_label + "\n"
    if limit_entry["has_fvg"]: msg += "✅ FVG alignee\n"
    if limit_entry["has_ob"]:  msg += "✅ OB aligne\n"
